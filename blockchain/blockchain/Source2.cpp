...
void hashavimas(vector<unsigned long>& reiksmes, unsigned long & H0, unsigned long& H1, unsigned long& H2, unsigned long& H3, unsigned long& H4, unsigned long& H5, unsigned long& H6, unsigned long& H7);
...
int main() {
	...
	int skaicius = reiksmes.size();
	if (skaicius > 55) {
		while (skaicius > 55) {

			int laik = reiksmes.size() / 2;
			for (int i = 0; i < laik; i++) {
				temp.push_back(reiksmes[i]);
			}
			didinimas(temp);
			rusiavimas(temp);
			hashavimas(temp, H0, H1, H2, H3, H4, H5, H6, H7);

			reiksmes.erase(reiksmes.begin(), reiksmes.begin() + laik);
			skaicius = reiksmes.size();
		...
		didinimas(reiksmes);
		rusiavimas(reiksmes);
		hashavimas(reiksmes, H0, H1, H2, H3, H4, H5, H6, H7);
